...
- **扫描性能优化**：通过随机化扫描IP顺序和引入延迟扫描，优化扫描过程，避免网络拥塞。
- **准确性提升**：改进主机可达性判断逻辑，综合多种方式检测；优化端口扫描机制，更准确识别开放端口。
- **超时控制**：增强全局扫描超时机制和单个任务的超时处理，防止扫描过程卡住。
- **稳定性提升**：改进了错误处理，完善了线程管理和资源释放，提高了应用在不同网络环境下的稳定性。
- **结果可靠性**：明确标记通过IP生成的伪MAC地址，区分模拟数据与真实获取的数据。
- **资源管理**：优化线程和内存使用，减少资源占用。
- **代码文档**：添加了完整的Doxygen风格注释，便于维护和开发。
- **UI响应性**：改进了UI反馈机制，提供更好的用户体验。
...
- **网络发现机制**：使用优化的算法探测局域网设备，包括自适应超时设置、随机化IP扫描顺序、智能重试机制以及引入扫描延迟避免网络风暴。
- **主机可达性判断**：综合尝试连接常用端口及判断是否处于同一子网等多种方式，提高主机存活检测的准确性。
- **端口扫描**：采用更智能的超时策略，并对不同类型的端口（如常见服务端口）设置不同的超时逻辑，区分端口关闭与防火墙过滤。
- **多线程扫描**：根据系统CPU核心数自动调整并行扫描线程数，并对并发任务进行有效管理。
- **伪MAC地址生成**：当无法通过系统ARP表获取真实MAC地址时，使用基于IP的一致性伪MAC地址生成，并在结果中明确标记为模拟数据（如 "SM:XX:XX:XX:XX"）。
- **超时控制**：全局扫描超时保护机制以及针对单个扫描任务的精细化超时管理，防止在网络环境较差时应用程序卡住。
- **代码文档**：完整的Doxygen格式代码注释，便于开发和维护。
...
2. **扫描卡在99%不动**
   - 已修复此问题，现在添加了120秒的全局超时限制
   - 如果仍然出现此问题，尝试减少自定义IP范围或使用"快速扫描"模式
...
### v2.2.0
- 增加了扫描超时控制，防止在部分网络环境下卡在99%
- 优化扫描性能，通过随机化IP和引入延迟扫描改进扫描机制
- 提高主机和端口检测的准确性与稳定性
- 改进错误处理机制和资源管理
- 明确标记伪MAC地址，提升结果可信度
- 更新代码以使用现代C++特性 (std::shuffle)
- 添加了完整的Doxygen格式代码注释
- 优化了MAC地址处理算法，增强了多种格式MAC地址的兼容性
...
项目GitHub：https://github.com/example/networkscanner
